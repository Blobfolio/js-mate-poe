...
frames:{type:Array,required:!0},audio:{type:[Object,null],required:!1,"default":null},useDefault:{type:Number,required:!1,"default":0},useEntrance:{type:Number,required:!1,"default":0},useFirst:{type:Number,required:!1,"default":0},flags:{type:Number,required:!1,"default":0},childId:{type:Number,required:!1,"default":0},edge:{type:[Array,null],required:!1,"default":null},next:{type:[Array,null],required:!1,"default":null}},methods:{playAnimation:function(a){this.$root.playAnimation(a)}},computed:{hasAudio:function(){return null!==
this.audio&&"string"===typeof this.audio.file},hasChild:function(){return 0<this.childId},hasEdge:function(){return null!==this.edge&&0<this.edge.length},hasNext:function(){return null!==this.next&&0<this.next.length},hasTree:function(){return this.hasChild||this.hasEdge||this.hasNext},isDependent:function(){return 4===this.id||5===this.id?!0:!this.useDefault&&!this.useFirst&&!this.useEntrance},duration:function(){if(this.variableDuration)return"(varies)";const a=this.steps.length;let b=0;for(let d=
0;d<a;++d)b+=this.steps[d].interval;return(new Intl.NumberFormat("en-US",{})).format(b)+"ms"},speed:function(){const a=`${this.start.speed}ms`,b=`${this.start.speed}ms`;return a===b?a:`${a} \u2013 ${b}`},steps:function(){let a=[];const b=this.frames.length,d=b+(b+this.repeatFrom)*this.repeat,c=this.end.speed-this.start.speed,m=this.end.x-this.start.x,n=this.end.y-this.start.y;let p=0-this.start.speed;for(let g=0;g<d;++g){const q=g/d,r=Math.floor(p+this.start.speed+c*q),A=r-p;p=r;a.push({step:g,time:r,
interval:A,frame:g<b?this.frames[g]:this.repeatFrom?this.frames[this.repeatFrom+(g-this.repeatFrom)%(b-this.repeatFrom)]:this.frames[g%b],x:this.start.x+m*q,y:this.start.y+n*q,audio:null})}return a}},template:'\n\t\t<div\n\t\t\tclass="animation"\n\t\t\t:id="\'animation-\' + id"\n\t\t>\n\t\t\t<h3 class="animation-title accent">\n\t\t\t\t<abbr\n\t\t\t\t\tclass="animation-name"\n\t\t\t\t\ttitle="View Details"\n\t\t\t\t>{{ name }}</abbr>\n\t\t\t\t<span class="animation-id">#{{ id }}</span>\n\t\t\t</h3>\n\n\t\t\t<div class="animation-legends">\n\t\t\t\t\x3c!-- Main attributes. --\x3e\n\t\t\t\t<div class="animation-legend">\n\t\t\t\t\t<poe-icon\n\t\t\t\t\t\tv-if="playable"\n\t\t\t\t\t\tclass="animation-legend-icon play-toggle"\n\t\t\t\t\t\ticon="play"\n\t\t\t\t\t\ttitle="Play Animation"\n\t\t\t\t\t\tv-on:click.prevent="playAnimation(id)"\n\t\t\t\t\t></poe-icon>\n\n\t\t\t\t\t<poe-icon\n\t\t\t\t\t\tv-if="hasAudio"\n\t\t\t\t\t\tclass="animation-legend-icon"\n\t\t\t\t\t\ticon="audio"\n\t\t\t\t\t\ttitle="Makes Noise"\n\t\t\t\t\t></poe-icon>\n\n\t\t\t\t\t<poe-icon\n\t\t\t\t\t\tv-if="isDependent"\n\t\t\t\t\t\tclass="animation-legend-icon"\n\t\t\t\t\t\ticon="link"\n\t\t\t\t\t\ttitle="Dependent Animation"\n\t\t\t\t\t></poe-icon>\n\n\t\t\t\t\t<poe-icon\n\t\t\t\t\t\tv-if="0 < useFirst"\n\t\t\t\t\t\tclass="animation-legend-icon"\n\t\t\t\t\t\ticon="startup"\n\t\t\t\t\t\ttitle="Startup Choice"\n\t\t\t\t\t></poe-icon>\n\n\t\t\t\t\t<poe-icon\n\t\t\t\t\t\tv-if="0 < useDefault"\n\t\t\t\t\t\tclass="animation-legend-icon"\n\t\t\t\t\t\ticon="default"\n\t\t\t\t\t\ttitle="Default Choice"\n\t\t\t\t\t></poe-icon>\n\n\t\t\t\t\t<poe-icon\n\t\t\t\t\t\tv-if="0 < useEntrance"\n\t\t\t\t\t\tclass="animation-legend-icon"\n\t\t\t\t\t\ticon="offscreen"\n\t\t\t\t\t\ttitle="Offscreen Choice"\n\t\t\t\t\t></poe-icon>\n\t\t\t\t</div>\n\n\t\t\t\t\x3c!-- Speed. --\x3e\n\t\t\t\t<div class="animation-legend">\n\t\t\t\t\t<poe-icon\n\t\t\t\t\t\tclass="animation-legend-icon"\n\t\t\t\t\t\ticon="speed"\n\t\t\t\t\t\ttitle="Playback Speed"\n\t\t\t\t\t></poe-icon>\n\n\t\t\t\t\t<div class="animation-legend-value">{{ speed }}</div>\n\t\t\t\t</div>\n\n\t\t\t\t\x3c!-- Time. --\x3e\n\t\t\t\t<div class="animation-legend">\n\t\t\t\t\t<poe-icon\n\t\t\t\t\t\tclass="animation-legend-icon"\n\t\t\t\t\t\ticon="time"\n\t\t\t\t\t\ttitle="Playback Duration"\n\t\t\t\t\t></poe-icon>\n\n\t\t\t\t\t<div class="animation-legend-value">{{ duration }}</div>\n\t\t\t\t</div>\n\t\t\t</div>\x3c!-- .animation-legends --\x3e\n\n\t\t\t<div\n\t\t\t\tclass="frames-list"\n\t\t\t\t:class="{ \'is-repeatable\' : repeat > 0 }"\n\t\t\t>\n\t\t\t\t<poe-frame\n\t\t\t\t\tv-for="(f, index) in frames"\n\t\t\t\t\t:key="\'frame-\' + id + \'-\' + index"\n\t\t\t\t\t:frame="f"\n\t\t\t\t\t:repeat="repeat > 0 && index >= repeatFrom"\n\t\t\t\t></poe-frame>\n\t\t\t</div>\n\n\t\t\t<div\n\t\t\t\tclass="animation-trees"\n\t\t\t\tv-if="hasTree && expanded"\n\t\t\t>\n\t\t\t\t<poe-tree\n\t\t\t\t\tv-if="hasChild"\n\t\t\t\t\t:key="\'tree-child-\' + id"\n\t\t\t\t\ttrunk="Child"\n\t\t\t\t\t:values="[{ weight: 1, id: childId }]"\n\t\t\t\t></poe-tree>\n\n\t\t\t\t<poe-tree\n\t\t\t\t\tv-if="hasNext"\n\t\t\t\t\t:key="\'tree-next-\' + id"\n\t\t\t\t\ttrunk="Next"\n\t\t\t\t\t:values="next"\n\t\t\t\t></poe-tree>\n\n\t\t\t\t<poe-tree\n\t\t\t\t\tv-if="hasEdge"\n\t\t\t\t\t:key="\'tree-edge-\' + id"\n\t\t\t\t\ttrunk="Edge"\n\t\t\t\t\t:values="edge"\n\t\t\t\t></poe-tree>\n\t\t\t</div>\n\t\t</div>\n\t'});
...
useDefault:b.a,useEntrance:b.c,useFirst:b.b,flags:b.flags,childId:b.f,edge:c,next:d});return a},[]).sort((a,b)=>`${a.primary?"0_":"1_"}${a.playable?"0_":"1_"}${a.name}`<`${b.primary?"0_":"1_"}${b.playable?"0_":"1_"}${b.name}`?-1:1),immersive:!1,tab:"animations",tabs:[{id:"animations",name:"Animations"},{id:"sprite",name:"Sprite"}]},mounted:function(){y||(y=!1)},methods:{immersiveToggle:function(a){this.immersive=!!a;document.body.classList.toggle("is-immersive",this.immersive);this.tab=this.immersive?
"immersive":"animations"},playAnimation:function(a){window.Poe.setAnimation(a)},summary:function(a){return void 0!==this.summaries[a]?this.summaries[a]:!1},tabToggle:function(a){this.tab=a}},computed:{summaries:function(){return this.animations.reduce((a,b)=>{a[b.id]={id:b.id,name:b.name,frame:b.frames[0]};return a},{})}}});}).call(this);
